...
- `` (): The value to send with the transaction (typically 0 for standard calls; non-zero allowed when intentionally forwarding native ETH).
- `` (): The gas limit for the transaction.
- `` (): The type of operation.
- `` (): The function selector of the handler/request function.
- `` (): The function selector to execute (NATIVE_TRANSFER_SELECTOR is a convenience selector for native-only transfers).
- `` (): The encoded parameters for the function (must be empty when using NATIVE_TRANSFER_SELECTOR).
...
- `` (): The SecureOperationState to check.
- `` (): Name of the role.
- `` (): Maximum number of wallets allowed for this role. Not capped by a protocol-wide constant;
...
**Security:** PROTECTED-ROLE POLICY: This library enforces the protected-role check for
...
```solidity
function registerFunction(struct EngineBlox.SecureOperationState self, string functionSignature, bytes4 functionSelector, string operationName, uint16 supportedActionsBitmap, bool enforceHandlerRelations, bool isProtected, bool isGrantRevocable, bytes4[] handlerForSelectors) public nonpayable
```
...
- `` (): When true (strict mode), handlerForSelectors in role permissions must match this schema&#x27;s handlerForSelectors at use time. When false (flexible mode), forward references are allowed.
- `` (): Whether the function schema is protected from **unregister** (&#x60;unregisterFunction&#x60;).
- `` (): When false, &#x60;removeFunctionFromRole&#x60; cannot remove this selector from any role; when true, grants may be removed from any role, including protected system roles.
...
**Security:** OPERATIONAL MODES: We do not require handlerForSelectors[i] to be in supportedFunctionsSet at registration.
...
Recovers the signer from the EIP-712 digest and signature. Uses standard EIP-712 recovery (no message prefix).
...
**Parameters:**
- `` (): The EIP-712 digest (keccak256(&quot;\x19\x01&quot; || domainSeparator || structHash))
- `` (): The signature (r, s, v)
...
### _validateMetaTxMatchRecord

```solidity
function _validateMetaTxMatchRecord(struct EngineBlox.SecureOperationState self, uint256 txId, struct EngineBlox.TxRecord metaTxRecord) internal view
```

Validates that the meta-transaction txRecord matches the stored record for the given txId.
...
**Parameters:**
- `` (): The SecureOperationState containing the stored tx record.
- `` (): The transaction ID.
- `` (): The TxRecord from the meta-transaction calldata.



---

### _validateMetaTxPaymentMatchRecord

```solidity
function _validateMetaTxPaymentMatchRecord(struct EngineBlox.SecureOperationState self, uint256 txId, struct EngineBlox.TxRecord metaTxRecord) internal view
```

Validates that the meta-transaction payment matches the stored record for the given txId.
...
### _schemaHasHandlerSelector

```solidity
function _schemaHasHandlerSelector(struct EngineBlox.FunctionSchema schema, bytes4 handlerSelector) internal view returns (bool)
```

Checks whether a given handler selector is present in a function schema's handlerForSelectors array.

**Parameters:**
- `` (): The function schema to inspect.
- `` (): The handler selector to search for.

**Returns:**
- True if the handler selector is present, false otherwise.
...
```solidity
event TransactionEvent(uint256 txId, bytes4 functionHash, enum EngineBlox.TxStatus status, address requester, address target, bytes32 operationType, bytes32 resultHash)
```
