...
### validateEmptyPayment

```solidity
function validateEmptyPayment(address recipient, uint256 nativeTokenAmount, address erc20TokenAddress, uint256 erc20TokenAmount) internal pure
```

Validates that payment details are empty (no attached payment).

**Parameters:**
- `` (): The payment recipient address
- `` (): The native token payment amount
- `` (): The ERC20 token address
- `` (): The ERC20 payment amount
...
### validateMetaTxHandlerSelectorBinding

```solidity
function validateMetaTxHandlerSelectorBinding(bytes4 handlerSelector, bytes4 entrySelector) internal pure
```

Validates that the signed handler selector matches the live wrapper entrypoint selector.
     IMPORTANT: pass the wrapper selector from wrapper context (`bytes4(msg.sig)` in the wrapper),
     not from external-library context.

**Parameters:**
- `` (): Selector embedded in signed MetaTxParams
- `` (): Selector of the wrapper entrypoint executing the meta-tx



---

### validateMetaTxHandlerContractBinding

```solidity
function validateMetaTxHandlerContractBinding(address handlerContract) internal view
```

Validates that the signed handler contract matches the active verifying contract (`address(this)`).

**Parameters:**
- `` (): Address embedded in signed MetaTxParams
...
Validates that a meta-transaction deadline has not passed (inclusive boundary: `block.timestamp == deadline` is valid).
...
**Parameters:**
- `` (): The deadline timestamp from &#x60;MetaTxParams&#x60;
...
Validates that the first value is not greater than the second (allows inclusive range: from <= to)
