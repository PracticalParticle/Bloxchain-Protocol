/**
 * Workflow SDK Test Runner
 * Main file to run workflow tests
 */

import { WorkflowCoreTests } from './workflow-core-tests';

class WorkflowSDKTestRunner {
  private testSuites: Record<string, typeof WorkflowCoreTests> = {
    core: WorkflowCoreTests,
    // Add more test suites here as they are created
    // builder: WorkflowBuilderTests,
    // transition: WorkflowTransitionTests,
    // extension: WorkflowExtensionTests,
    // integration: WorkflowIntegrationTests,
  };
  ...
  parseArguments(): string[] | null {
    ...
    if (args.includes('--all')) {
      selectedSuites.push(...Object.keys(this.testSuites));
    } else {
      if (args.includes('--core')) selectedSuites.push('core');
      // Add more options as test suites are added
    }
  ...
  async runTestSuite(suiteName: string, TestClass: typeof WorkflowCoreTests): Promise<boolean> {
  ...
  async runTests(selectedSuites: string[]): Promise<boolean> {
    ...
    console.log('🔧 Workflow SDK Test Runner Starting...');
    console.log('='.repeat(60));
  ...
  printFinalResults(suiteResults: Record<string, boolean>): void {
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📊 WORKFLOW SDK TEST RUNNER FINAL RESULTS');
    ...
    if (this.results.failedSuites === 0) {
      ...
      console.log('🚀 Workflow SDK is working perfectly!');
    } else {
...
if (require.main === module) {
  const runner = new WorkflowSDKTestRunner();
  runner.run();
}

export { WorkflowSDKTestRunner };
